...
#include <ctype.h>
#include <stack>
#include <exception>  // added 01/22/2012
#include <stdexcept>  // added 01/22/2012
...
std::string XmlReader::element()
{
  ...
  {
    throw std::logic_error("malformed XML");  // replaced exception with logic_error 01/22/2012
  }
...
std::string XmlReader::body()
{
  ...
  if(locpos1 >= elem.size())
    throw std::logic_error("malformed XML");
  if(elem[locpos1-1] == '/')
    return "";
...
XmlReader::attribElems XmlReader::attributes()
{
  ...
  while(true)
  {
    ...
    if(locpos < localposition)
      throw std::logic_error("malformed XML");  // modified 01/22/2012
